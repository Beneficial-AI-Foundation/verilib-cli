...
fn save_config(
    ...
    let mut config_json_value = if config_path.exists() {
        ...
        existing_json["repo"] =
            serde_json::to_value(&repo_config).context("Failed to serialize repo config")?;
    ...
    // Add execution-mode
    config_json_value["execution-mode"] =
        serde_json::to_value(&execution_mode).context("Failed to serialize execution mode")?;
