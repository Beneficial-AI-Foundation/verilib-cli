...
def discover_function_in_module(
    ...
    print(f"  Looking for module: {module} in src_dir: {src_dir}")
    
    # Logic to normalize the module path (strip crate name if present)
    if "::" in module:
        parts = module.split("::")
        # Heuristic: if the first part looks like a crate name (and doesn't match CRATE_NAME, or does), strip it
        # This is because probe-verus output usually includes the crate name
        print(f"  Stripping crate prefix {parts[0]}")
        module_stripped = "::".join(parts[1:])
    elif module == CRATE_NAME:
         module_stripped = ""
    else:
        module_stripped = module
    ...
    # 1. Standard structure: src_dir/CRATE_DIR/src/...
    base_path_1 = src_dir / CRATE_DIR / "src"
    if module_parts:
         possible_paths.append(base_path_1 / "/".join(module_parts[:-1]) / f"{module_parts[-1]}.rs")
         possible_paths.append(base_path_1 / "/".join(module_parts) / "mod.rs")
    
    # 2. Try simple structure: src_dir/src/... (if we are already inside the crate)
    base_path_2 = src_dir / "src"
    if module_parts:
         possible_paths.append(base_path_2 / "/".join(module_parts[:-1]) / f"{module_parts[-1]}.rs")
         possible_paths.append(base_path_2 / "/".join(module_parts) / "mod.rs")
         
    # 3. Try module-root structure: src_dir/module_part0/src/... (monorepo style?)
    if len(module_parts) > 1:
        base_path_3 = src_dir / module_parts[0] / "src" 
        sub_parts = module_parts[1:]
        possible_paths.append(base_path_3 / "/".join(sub_parts[:-1]) / f"{sub_parts[-1]}.rs")
        possible_paths.append(base_path_3 / "/".join(sub_parts) / "mod.rs")
...
def main():
    ...
    # Check if REPO_ROOT env var is set (e.g. from Docker)
    if 'REPO_ROOT' in os.environ:
        repo_root = Path(os.environ["REPO_ROOT"])
        print(f"Using REPO_ROOT from environment: {repo_root}")
    else:
        repo_root = script_dir.parent  # Go up one level from scripts/ to repo root
        print(f"Using calculated REPO_ROOT: {repo_root}")
    ...
    if not seed_path.exists():
        ...
        # In Docker, paths might be mounted differently. List files for debugging
        print(f"Listing files in {repo_root}:")
        try:
...
if __name__ == "__main__":
    ...
    if os.environ.get("CRATE_NAME"):
        CRATE_NAME = os.environ.get("CRATE_NAME")
    
    if os.environ.get("CRATE_DIR"):
        CRATE_DIR = os.environ.get("CRATE_DIR")
